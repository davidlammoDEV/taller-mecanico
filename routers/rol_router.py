...
def crear_roles(rol: RolEntrada, db:Session = Depends(get_db)):
    return rol_service.crear_roles(rol, db)

@rol_router.patch("/{idrol}", response_model=RolSalida)
def actualizar_rol_parcial(idrol: int, rol_up: RolUpdata, db: Session = Depends(get_db)):
...
def eliminar_rol_logico(idrol: int, db: Session = Depends(get_db)):
