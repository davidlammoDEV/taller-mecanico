...
def listar_usuarios_rol(db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
    return user_login_service.listar_usuarios_rol(db)

@user_log_router.post("/", status_code=status.HTTP_201_CREATED)
def crear_usuario_rol(userol: User_LogEntrada, db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
...
def actualizar_usuario_rol(
    ...
    current_user: Usuario = Depends(VerificarRoles([2]))
):
...
def eliminar_usuario_rol(iduser_log: int, db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
