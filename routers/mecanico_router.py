...
def obtener_mecanico(id:int, db:Session = Depends(get_db), current_user = Depends(require_cliente)):
...
def listar_mecanicos(db: Session = Depends(get_db), current_user = Depends(require_cliente)):
...
def crear_mecanico(mecanico: MecanicoEntrada, db:Session = Depends(get_db), current_user = Depends(require_mecanico)):
    return mecanico_service.crear_mecanico(mecanico, db)

@mecanico_router.put("/{id}", response_model=MecanicoSalida)
def actualizar_mecanico_completo(id: int, cliente_update: MecanicoEntrada, db: Session = Depends(get_db), current_user = Depends(require_mecanico)):
...
def actualizar_mecanico_parcial(id: int, mecanico_updata: MecanicoUpdate, db: Session = Depends(get_db), current_user = Depends(require_mecanico)):
...
def eliminar_mecanico_logico(id: int, db: Session = Depends(get_db), current_user = Depends(require_supervisor)):
