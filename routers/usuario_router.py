...
def listar_usuarios(db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
    return usuario_service.listar_usuarios(db)

@usuario_router.post("/", response_model=User_Entrada, status_code=status.HTTP_200_OK)
def crear_proveedor(user: User_Entrada, db:Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
    return usuario_service.crear_usuario(user, db)

@usuario_router.patch("/{iduser}", response_model=User_Salida)
def actualizar_usuario_parcial(iduser: str, user_update: User_Updata, db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
...
def eliminar_usuario_logico(iduser: str, db: Session = Depends(get_db), current_user: Usuario = Depends(VerificarRoles([2]))):
